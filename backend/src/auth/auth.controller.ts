...
export class AuthController {
  ...
  @ApiResponse({ status: 201, description: 'Returns access token; sets httpOnly refresh cookie' })
  ...
  @ApiResponse({ status: 200, description: 'Returns access token; sets httpOnly refresh cookie' })
  ...
  @ApiResponse({ status: 200, description: 'Returns new access token; rotates refresh cookie' })
  @ApiResponse({ status: 401, description: 'Missing or invalid refresh cookie' })
  async refresh(
    @Request() req: AuthenticatedRequest,
    @Response({ passthrough: true }) res: ExpressResponse,
  ) {
    const tokens = await this.authService.refresh(req.user.userId, req.user.email);
    res.cookie(REFRESH_COOKIE, tokens.refreshToken, COOKIE_OPTIONS);
